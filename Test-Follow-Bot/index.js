...
(request = require('request')),
	...
	(admins = ['780510852457431060', '', '']),
	(config = {
		'850442592021905448': 500,
		'850442457236635649': 100,
		'850442351628386305': 75,
		'850441275290550334': 10,
		'850469506954690586': 5,
	});
...
client.on('message', async message => {
	...
	if (command === 'help') {
		message.channel.send(
			new Discord.MessageEmbed().setColor('0x7d06cc').addFields(
				{
	...
	if (command === 'buy') {
		message.channel.send(
			new Discord.MessageEmbed()
				.setDescription(`price 1 nitro classic, DM GeneralOG#8242 for more info`)
				.setColor('PURPLE')
				.setFooter(
					message.guild.name,
					message.guild.iconURL({
		...
		if (!message.member.hasPermission('MANAGE_CHANNELS'))
			return message.channel.send(
				new Discord.MessageEmbed()
					...
					.setColor(0xcff9ff)
					.setImage(
						'https://cdn.discordapp.com/attachments/836044564158611486/836058814176034816/unknown.png'
					)
			);
	...
	if (command === 'lock') {
		...
		message.channel.send(
			new Discord.MessageEmbed()
				.setImage(
					'https://cdn.discordapp.com/attachments/833139865352273930/835651790149517370/image0.gif'
				)
		...
		if (admins.includes(message.author.id)) {
			if (!args[0])
				return message.channel.send(
					new Discord.MessageEmbed()
						.setColor(0x7d06cc)
						.setDescription('You must specify a twitch username !')
				);

			var roleID = Object.entries(config).find(([key, value]) =>
			...
			await getUser(args[0]).then(res => {
				if (res._total === 0) {
					return message.channel.send(
						new Discord.MessageEmbed()
							.setColor(0x7d06cc)
							.setDescription('You must specify **valid** a twitch username !')
					);
				} else {
					...
					channel
						.send(
							new Discord.MessageEmbed()
								...
								.setDescription(
									`Successfully added **${number}** followers to \`${
										args[0]
									}\` (Twitch ID: \`${twitchID}\`)\n\nCheck out [${
										args[0]
									}'s twitch channel](https://twitch.tv/${args[0]}/)`
								)
						)
						.then(msg => {
			...
			message.channel.send(
				new Discord.MessageEmbed()
					.setColor('0x7d06cc')
					.setDescription(`Adding **${number}** followers to \`${args[0]}\` !`)
			);
		} else if (!admins.includes(message.author.id)) {
			if (
				message.channel.id ===
				client.channels.cache.find(c => c.name === chatchannel).id
			)
...
`;
		...
		request.post(options, (err, res, body) => {
			if (err) {
				return console.log(
					`Invalid token try with another tokens like tyogihfp9rueyo7vt4ple2wlihmlic`
				);
			}
...
client.on('guildMemberAdd', async member => {
	...
	for (let i = 0; i < channels.length; i++) {
		...
		if (channel)
			channel
				.send(`${member}, **Check out this channel !**`)
				.then(async msg => {
					msg.delete({
						timeout: 5000
					});
